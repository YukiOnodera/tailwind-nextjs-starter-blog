...
- [x] (Optional) Newsletter
  - emailoctopus をとりあえず検証
  - メール送信時に送付元の住所などの情報が必要になるため、なくても良さそう
  - 審査通ったらとりあえずつけても良さそう
  - メール送信時の内容なども作り込まなきゃいけなそうだから、形だけできればそれでよし
  - 審査がめんどくさそうなため、これはやめる
- [x] (Optional) 英語記事出力の更なる自動化、手順化、テンプレ化、mdx の利用も視野に入れる
  - Front Matter, JS などを使って、VSCodeでmdx 開発がうまくできないか、色々試す
